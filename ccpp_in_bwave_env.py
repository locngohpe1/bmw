...
'''
    ...
    def check_actual_dynamic_collision(self, target_pos):
        ...
        # Check if target position is actually occupied by a moving dynamic obstacle
        for obstacle in self.dynamic_obstacles.obstacles:
            obstacle_center = obstacle['pos']  # (row, col)
            obstacle_size = obstacle.get('size', 1.0)

            # Calculate distance from target to obstacle center
            distance = math.sqrt((target_y - obstacle_center[0]) ** 2 +
                                 (target_x - obstacle_center[1]) ** 2)

            # Determine collision radius based on obstacle size
            if isinstance(obstacle_size, tuple):
                collision_radius = max(obstacle_size) / 2
            else:
                collision_radius = obstacle_size / 2

            # If within collision radius, it's a real collision
            if distance <= collision_radius + 0.5:  # Small buffer
                print(f"🚫 REAL dynamic collision detected at ({target_x}, {target_y}) with obstacle {obstacle['id']}")
                print(f"   Distance: {distance:.2f}, Collision radius: {collision_radius:.2f}")

                # Apply waiting rule for actual dynamic obstacles
                wait_time = 1.0 + (collision_radius - 1.0) * 0.5  # Scale wait time with size

                print(f"⏳ Waiting {wait_time:.1f}s for dynamic obstacle to pass...")
                time.sleep(wait_time)  # Simple waiting

                return True

        # Check if it's just a stale mark (old trace)
        if (0 <= target_y < len(self.ui.map) and 0 <= target_x < len(self.ui.map[0]) and
                self.ui.map[target_y][target_x] == 'd'):
            print(f"🧹 Ignoring stale dynamic mark at ({target_x}, {target_y}) - no actual obstacle")
            # Clean up the stale mark
            self.ui.map[target_y][target_x] = 0
            return False
